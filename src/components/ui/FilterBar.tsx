...
export function FilterBar({
  ...
  return (
    <div
      ref={filterBarRef}
      className={`fixed bottom-0 left-0 right-0 z-sticky transition-transform duration-300 ease-out ${
      ...
      >
        <div className="max-w-[1440px] mx-auto px-4 md:px-8 py-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            ...
            {/* Price */}
            <div>
              <h4 className="text-[10px] uppercase tracking-widest text-alpha/60 mb-3 font-primary">Price</h4>
              <div className="space-y-2">
                {priceRanges.map(option => (
                  <button
                    ...
                    className={`block w-full text-left text-sm font-primary py-1 transition-colors ${
                      selectedPrice === option.value ? "text-alpha font-medium" : "text-alpha/60 hover:text-alpha"
                    }`}
                  >
                    {option.label}
                  </button>
            ...
            <div className="hidden md:flex items-center gap-6">
              ...
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs uppercase tracking-widest font-primary hover:text-creme/80 transition-colors"
                >
                  <span>Price: {priceRanges.find(o => o.value === selectedPrice)?.label}</span>
                  ...
                  <div className="absolute bottom-full left-0 mb-2 bg-creme text-alpha shadow-dropdown min-w-[180px] py-2">
                    {priceRanges.map(option => (
                      <button
                        ...
                        onClick={() => handlePriceChange(option.value)}
                        className={`block w-full text-left px-4 py-2 text-xs font-primary hover:bg-alpha/5 transition-colors ${
                          selectedPrice === option.value ? "font-medium" : ""
                        }`}
                      >
                        {option.label}
                      </button>
